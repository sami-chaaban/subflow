...
def displaycomplex(output_text_step12, entry_pickedmics_dir, entry_picks_dir, entry_todisplay_step12, selected_order_step12, selected_picktype, filter_forcomplex_checkbox, entry_config):
    global pickedmics
    picktype = selected_picktype.get()
    picks_dir = entry_picks_dir.get()
    pickedmics = entry_pickedmics_dir.get()
    order = selected_order_step12.get()
    filterflag = filter_forcomplex_checkbox.get()
    ...
    output_text_step12.delete(1.0, tk.END)  # Clear previous output
    output_text_step12.insert(tk.END, "BoxManager started...\n")
    output_text_step12.see(tk.END)

    if not os.path.exists(pickedmics):
        output_text_step12.insert(tk.END, f"Cannot find the directory {pickedmics}.\n")
        output_text_step12.see(tk.END)
    ...
    def run_boxmanager():
        ...
        if len(miclist) == 0:
            output_text_step12.insert(tk.END, f"Error: There are no picked micrographs.\n")
            output_text_step12.see(tk.END)
            return

        todisplay = entry_todisplay_step12.get()

        if todisplay not in ["all", "All", "ALL"]:
            if int(todisplay) > len(miclist):
                output_text_step12.insert(tk.END, f"Warning: There are only {len(miclist)} picked micrographs.\n")
                output_text_step12.see(tk.END)
        ...
        try:
            ...
            for line in process.stdout:
                output_text_step12.insert(tk.END, line)
                output_text_step12.see(tk.END)
            ...
            output_text_step12.insert(tk.END, "BoxManager completed.\n")
            output_text_step12.see(tk.END)

        except subprocess.CalledProcessError as e:
            # Display an error message if the subprocess fails
            output_text_step12.insert(tk.END, f"Error running BoxManager: {e.stderr}\n")
            output_text_step12.see(tk.END)
